...
func Inject(homeDir string, agent model.AgentID) (InjectionResult, error) {
	...
	writeResult, err := mergeJSONFile(settingsPath, personaOverlayJSON)
	if err != nil {
		return InjectionResult{}, err
	}
...
func settingsPathForAgent(homeDir string, agent model.AgentID) (string, error) {
	...
	default:
		return "", fmt.Errorf("persona injector does not support agent %q", agent)
	}
}
