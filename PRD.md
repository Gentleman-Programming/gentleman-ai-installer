...
```mermaid
flowchart TD
    START([gentleman-ai install]) --> DETECT
    ...
    subgraph PHASE_3["Phase 3: Backup"]
        BACKUP[Backup existing configs<br/>~/.gentleman-ai-backup-TIMESTAMP/]
    end
...
| Homebrew | `brew install Gentleman-Programming/tap/gentleman-ai` | P0 |
| Go install | `go install github.com/Gentleman-Programming/gentleman-ai-installer/cmd/gentleman-ai@latest` | P1 |
| Direct binary | Download from GitHub Releases | P1 |
| winget (Windows) | `winget install gentleman-ai` | P2 |
...
**Requirements:**
- R-UPDATE-01: The installer MUST support `gentleman-ai update` to check for and install newer versions of itself
- R-UPDATE-02: The installer MUST support `gentleman-ai update --skills` to pull latest skill versions for all configured agents
- R-UPDATE-03: The installer MUST support `gentleman-ai update --engram` to update Engram to the latest version
...
- R-REL-03: The installer MUST support `gentleman-ai repair` to re-run failed steps
...
1. **Naming**: `gentleman-ai`, `gentleman-ai-installer`, `gai`, or something else? Should it be part of the `Gentleman-Programming` org or standalone?
...
gentleman-ai install --preset gentleman --agents claude-code,opencode

# Minimal setup, just Claude Code with basic security
gentleman-ai install --preset minimal --agents claude-code

# Team provisioning from shared profile
gentleman-ai install --profile ./team-ai-config.yaml

# Update all skills to latest
gentleman-ai update --skills

# Update Engram
gentleman-ai update --engram

# Backup current configs
gentleman-ai backup

# Restore from backup
gentleman-ai restore --list
gentleman-ai restore --id 2026-02-27-143022
