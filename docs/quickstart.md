...
### Fedora/RHEL (and derivatives like CentOS, Rocky Linux, AlmaLinux, Nobara)
...
If you run the installer on an unsupported OS or Linux distro (e.g., Gentoo, NixOS), it exits immediately with an error:

- `unsupported operating system: only macOS, Linux, and Windows are supported (detected <os>)`
- `unsupported linux distro: Linux support is limited to Ubuntu/Debian, Arch, and Fedora/RHEL (detected <distro>)`
